...
const P = 'jg'
const STORAGE_KEY = 'jingou-chats'
        ...
        <div className={`${P}-chat-header`}>
          {char.portrait.startsWith('/') ? (
            ...
            <span style={{
              ...
              fontSize: 20, background: 'var(--bg-hover)',
            }}>
              {char.portrait}
            </span>
          )}
        ...
        <div className={`${P}-chat-area`} style={{ flex: 1 }}>
          {messages.length === 0 && !streaming && (
            <div className={`${P}-placeholder`}>
              <div className={`${P}-placeholder-icon`}>
                {char.portrait.startsWith('/') ? '💬' : char.portrait}
              </div>
              <p>和{char.name}开始聊天吧</p>
            </div>
          )}
          ...
          {messages.map((msg, i) =>
            msg.role === 'user' ? (
              ...
              <div key={i} className={`${P}-bubble-npc`} style={{ borderLeftColor: char.themeColor }}>
                {msg.content}
              </div>
            ),
          )}
          ...
          {streaming && streamingContent && (
            <div className={`${P}-bubble-npc`} style={{ borderLeftColor: char.themeColor }}>
              {streamingContent}
            </div>
          )}
